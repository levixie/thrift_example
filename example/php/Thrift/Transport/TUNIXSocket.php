...
/**
 * UNIX sockets implementation of the TTransport interface.
 *
 * @package thrift.transport
 */
class TUNIXSocket extends TTransport {
   ...
   * @var string
   */
  protected $filename_ = 'socket.sock';
   ...
   * Socket constructor
   *
   * @param string $filename     Socket filename
   * @param bool   $persist      Whether to use a persistent socket
   * @param string $debugHandler Function to call for error logging
   */
  public function __construct($filename='socket.sock',
                              $persist=FALSE,
                              $debugHandler=null) {
  ...
  /**
   * Get the filename that this socket is connected to
   *
  ...
  public function open() {
    ...
    if (empty($this->filename_)) {
      throw new TTransportException('Cannot open null filename', TTransportException::NOT_OPEN);
    }

    if ($this->persist_) {
      $this->handle_ = @pfsockopen("unix://".$this->filename_,
                                   -1,
                                   $errno,
                                   $errstr,
                                   $this->sendTimeoutSec_ + ($this->sendTimeoutUsec_ / 1000000));
    } else {
      $this->handle_ = @fsockopen("unix://".$this->filename_,
                                  -1,
                                  $errno,
                                  $errstr,
                                  $this->sendTimeoutSec_ + ($this->sendTimeoutUsec_ / 1000000));
    ...
    if ($this->handle_ === FALSE) {
      $error = 'TUNIXSocket: Could not connect to '.$this->filename_.' ('.$errstr.' ['.$errno.'])';
      if ($this->debug_) {
  ...
  public function read($len) {
    ...
    if ($readable > 0) {
      ...
      if ($data === false) {
          throw new TTransportException('TUNIXSocket: Could not read '.$len.' bytes from '.
                               $this->filename_);
      } elseif($data == '' && feof($this->handle_)) {
          throw new TTransportException('TUNIXSocket read 0 bytes');
        }

      return $data;
    } else if ($readable === 0) {
        throw new TTransportException('TUNIXSocket: timed out reading '.$len.' bytes from '.
                             $this->filename_);
      } else {
        throw new TTransportException('TUNIXSocket: Could not read '.$len.' bytes from '.
                             $this->filename_);
    ...
    while (TStringFuncFactory::create()->strlen($buf) > 0) {
      ...
      if ($writable > 0) {
        ...
        $buf = TStringFuncFactory::create()->substr($buf, $written);
          ...
          throw new TTransportException('TUNIXSocket: timed out writing '.TStringFuncFactory::create()->strlen($buf).' bytes from '.
                               $this->filename_);
        } else {
            throw new TTransportException('TUNIXSocket: Could not write '.TStringFuncFactory::create()->strlen($buf).' bytes '.
                                 $this->filename_);
